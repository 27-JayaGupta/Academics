...
def get_CSVMGrad(theta, t, C, X, y):
    w = theta[0:-1]
    b = theta[-1]
    n = y.size
    g = np.zeros((n,))
    discr = np.multiply((X.dot(w) + b), y)
    g[discr < 1] = -1

    b_grad = C * g.dot(y)
    w_grad = w + C * (X.T * g).dot(y)
    return np.append(w_grad, b_grad)    
    
def get_CSVMMBGrad(theta, t, C, X, y, b_size):
    w = theta[0:-1]
    b = theta[-1]
    n = min(b_size, y.size)
    k = y.size/n
...
################################
		...
		# grad=get_CSVMGrad(theta,t,C,X,y)
		grad=get_CSVMMBGrad(theta,t,C,X,y,batch_size)
