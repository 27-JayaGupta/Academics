...
int main()
{
  ...
  if(childProcess == 0)
  {
    if(flag == 2)
    {
      ...
      do_encryption(chunk_numbers, UNSET, SET);
  ...
  if(childProcess == 0)
  {
    ...
    {
      int chunk_numbers = 50;
      printf("Child process 3 with %d \n", chunk_numbers);
      do_encryption(chunk_numbers, SET, UNSET);
      printf("Child process 3 executed\n");
      exit(0);
    }
  }

  printf("Parent process 2 with %d \n", 50);
  do_encryption(50, UNSET, UNSET);
