 ...
 * TC:VMA with multiple physical allocations
*/
...
int main()
{
   ...
   ip.addr = (unsigned long)ptr+ONEGB;
   ip.length = (ONEGB>>12);
   ip.buff = mapping;
   ...
   unsigned addr_index = ONEGB+4096;
   struct address * temp;
   unsigned page_index = (&ptr[addr_index]-ptr)>>12;
   unsigned page_offset = ((unsigned long)&ptr[addr_index])&0xfff;
   ...
   printf("old address:%lx, new address:%lx\n",(unsigned long)&ptr[addr_index],new_addr);
   if(((unsigned long)ptr <= new_addr) && (new_addr < ((unsigned long)ptr+ONEGB))){
       printf("Testcase Failed\n");
       return -1;
   }
   close(fd);
