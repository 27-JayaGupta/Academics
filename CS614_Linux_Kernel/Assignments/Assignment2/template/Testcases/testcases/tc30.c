 ...
 * TC:VMA with different permissions, mprotect to change permission
*/
...
int main()
{
   ...
   ptr = (char*)mmap(NULL, SIZE, PROT_WRITE, MAP_ANONYMOUS|MAP_PRIVATE, 0, 0);
   ptr1 = (char*)mmap(ptr, SIZE, PROT_READ, MAP_ANONYMOUS|MAP_PRIVATE, 0, 0);
   ...
   if(ptr == MAP_FAILED || dt == MAP_FAILED || ptr1 == MAP_FAILED){
        perror("mmap");
        exit(-1);
   }
