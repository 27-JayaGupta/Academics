...
file_exists = exists("tc30.out")
empty=(os.stat("tc30.out").st_size == 0)
if(not file_exists or empty):
    print("Failed")
    exit()

d = {"before":{"pmd":[],"pte":[]},"after":{"pmd":[],"pte":[]}}
with open("tc30.out","r") as f1:
    for line in f1:
        line = line.strip("\n")
...
result = all((x!=y and (int(y,base=16)>>7&1))for x,y in zip(d["before"]["pmd"], d["after"]["pmd"]))
if(result):
    print("Passed")
else:
    print("Failed")
