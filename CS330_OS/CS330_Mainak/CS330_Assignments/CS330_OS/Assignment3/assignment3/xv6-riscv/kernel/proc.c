...
struct barrier barrier_list[BARRIER_COUNT];

// Condition Variable implementation of multi cons-prod bounded buffer
#define BUFFER_COUNT 20
struct buffer_elem buffer[BUFFER_COUNT];
int head = 0;   // index from where the consumer consumes
...
// initialize barrier_list at boot time
void
barrierinit(void)
{
  struct barrier* b;
  int i = 0;
  for(b = barrier_list; b< &barrier_list[BARRIER_COUNT]; b++) {
    b->state = 0;
    b->id = i++;
    b->num_proc_count = 0;
    initsleeplock(&b->lk, "barrier");
    cond_init(&b->cv, "barrier_cond");
  }
}
...
{ 
  ...
  return -1; // if no free barrier present
}

void
barrier(int barrier_instance_num, int barrier_id, int num_proc)
{ 
  acquiresleep(&print_lock);
  acquire(&myproc()->lock);
  printf("%d: Entered barrier#%d for barrier array id %d\n", myproc()->pid, barrier_instance_num, barrier_id);
  release(&myproc()->lock);
  releasesleep(&print_lock);

  struct barrier* b = &barrier_list[barrier_id];
  acquiresleep(&b->lk);
  b->num_proc_count += 1;
  if(b->num_proc_count == num_proc) {
    b->num_proc_count = 0;
    cond_broadcast(&b->cv);
  } else {
    cond_wait(&b->cv, &b->lk);
...
void 
buffer_cond_init(void)
{ 
  struct buffer_elem* b;
  for(b = buffer; b< &buffer[BUFFER_COUNT]; b++) {
    b->x = -1;
    b->full = 0;
    initsleeplock(&b->lk, "buffer_lock");
    cond_init(&b->inserted, "barrier_cond_inserted");
    cond_init(&b->deleted, "barrier_cond_deleted");
  }

  head = 0;
  tail = 0;
  initsleeplock(&grab_head, "grab_head");
  initsleeplock(&grab_tail, "grab_tail");
}

void 
cond_produce(int produce_val)
{ 
  // grab a index
  acquiresleep(&grab_tail);
  int index = tail;
  tail = (tail+1)%BUFFER_COUNT;
  releasesleep(&grab_tail);

  acquiresleep(&buffer[index].lk);
  while(buffer[index].full) {
      cond_wait(&buffer[index].deleted, &buffer[index].lk);
  }
  buffer[index].x = produce_val;
  buffer[index].full = 1;
  cond_signal(&buffer[index].inserted);
  releasesleep(&buffer[index].lk);
}
...
{ 
  int consumed_val;
  // grab a index
  acquiresleep(&grab_head);
  int index = head;
  head = (head+1)%BUFFER_COUNT;
  releasesleep(&grab_head);

  acquiresleep(&buffer[index].lk);
  while(!buffer[index].full) {
      cond_wait(&buffer[index].inserted, &buffer[index].lk);
  }
  consumed_val = buffer[index].x;
  buffer[index].full = 0;
  cond_signal(&buffer[index].deleted);
  releasesleep(&buffer[index].lk);

  acquiresleep(&print_lock);
  printf("%d ", consumed_val);
  releasesleep(&print_lock);
...
{
  struct sem_buffer_elem* b;
  
  for(b = sem_buffer_list; b < &sem_buffer_list[SEM_BUFFER_COUNT]; b++){
    b->x = -1;
  }
  ...
  sem_init(&pro, 1);
  sem_init(&con, 1);
  sem_init(&empty, SEM_BUFFER_COUNT);
  sem_init(&full, 0);
}

void
sem_produce(int produce_val)
{
  sem_wait(&empty);
  sem_wait(&pro);
  sem_buffer_list[nextp].x = produce_val;
  nextp = (nextp + 1) % SEM_BUFFER_COUNT;
  sem_post(&pro);
  sem_post(&full);
}
...
{ 
  ...
  sem_wait(&full);
  sem_wait(&con);
  consumer_val = sem_buffer_list[nextc].x;
  nextc = (nextc + 1) % SEM_BUFFER_COUNT;
  sem_post(&con);
  sem_post(&empty);

  acquiresleep(&print_lock);
  printf("%d ", consumer_val);
  releasesleep(&print_lock);
