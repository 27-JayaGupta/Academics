...
sys_barrier(void)
{
  // Add variable checking here if needed.
  int barrier_instance_num, barrier_id, num_proc;

  if(argint(0, &barrier_instance_num) < 0) 
    return -1;
  
  if(argint(1, &barrier_id) < 0) 
    return -1;
  
  if(argint(2, &num_proc) < 0) 
    return -1;
  
  barrier(barrier_instance_num, barrier_id, num_proc);
  return 0;
}
...
sys_cond_produce(void)
{
  ...
  if(argint(0, &produce_val) < 0) 
    return -1;

  cond_produce(produce_val);
  return 0;
}
